	...
	<div class="addEdit-block" :style='{"padding":"0px 0px 30px","fontSize":"14px","color":"#000","background":"none"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="入库数量" prop="shuliang">
					<el-input  v-model.number="ruleForm.shuliang" placeholder="入库数量" clearable  :readonly="ro.shuliang"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="入库数量" prop="shuliang">
					<el-input v-model="ruleForm.shuliang" placeholder="入库数量" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="date" v-if="type!='info'" label="入库时间" prop="rukushijian">
					<el-date-picker
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-else-if="ruleForm.rukushijian" label="入库时间" prop="rukushijian">
	...
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				this.ruleForm.rukubeizhu='用户归还入库'
			}
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `haishuruku/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.haishurukuCrossAddOrUpdateFlag = false;
						 this.parent.search();
