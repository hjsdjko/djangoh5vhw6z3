		...
		>
			<template >
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else-if="ruleForm.content" label="评论内容" prop="content">
				    <span :style='{"padding":"0 20px","color":"inherit","background":"#edf7ff","display":"inline-block","width":"80%","fontSize":"14px","lineHeight":"40px","fontWeight":"500"}' v-html="ruleForm.content"></span>
				</el-form-item>
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else-if="ruleForm.reply" label="回复内容" prop="reply">
				    <span :style='{"padding":"0 20px","color":"inherit","background":"#edf7ff","display":"inline-block","width":"80%","fontSize":"14px","lineHeight":"40px","fontWeight":"500"}' v-html="ruleForm.reply"></span>
				</el-form-item>
...
export default {
    ...
    info(id) {
      this.$http({
        url: `discussgonggongshuji/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `discussgonggongshuji/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.discussgonggongshujiCrossAddOrUpdateFlag = false;
						 this.parent.search();
