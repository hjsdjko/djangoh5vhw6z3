<template>
	<div class="main-content" :style='{"color":"#666","padding":"0px 30px 30px","fontSize":"14px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","margin":"0 0 10px","borderRadius":"0 0 8px 8px","textAlign":"left","background":"#fff","display":"block","width":"100%","fontSize":"inherit","order":"1"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="健康指标" prop="jiankangzhibiao">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">健康指标</label>
						<el-select clearable v-model="searchForm.jiankangzhibiao" placeholder="请选择健康指标" >
							<el-option v-for="(item,index) in jiankangzhibiaoOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"margin":"0px 0 20px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit","order":"2"}'>
					<el-button class="add" v-if="isAuth('shentijiankang','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('shentijiankang','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="btn18" v-if="isAuth('shentijiankang','血压统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						血压统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('shentijiankang','血糖统计')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						血糖统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('shentijiankang','每日血压')" type="success" @click="chartDialog3()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						每日血压
					</el-button>
				</el-row>
			...
			<div :style='{"border":"0px solid #eee","width":"100%","padding":"0","margin":"0px 0 0","borderRadius":"0px","background":"rgba(255,255,255,.9)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="baogaodan" width="200" label="报告单">
						<template slot-scope="scope">
							<div v-if="scope.row.baogaodan">
								<img v-if="scope.row.baogaodan.substring(0,4)=='http'" :src="scope.row.baogaodan.split(',')[0]" width="100" height="100">
								<img v-else :src="$base.url+scope.row.baogaodan.split(',')[0]" width="100" height="100">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="jiankangzhibiao"
						label="健康指标">
						<template slot-scope="scope">
							{{scope.row.jiankangzhibiao}}
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('shentijiankang','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('shentijiankang','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('shentijiankang','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="xueyaChart3" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
//统计接口
    chartDialog3() {
      ...
      this.$nextTick(()=>{
        ...
        var xueyaChart3 = echarts.init(document.getElementById("xueyaChart3"),'macarons');
        this.$http({
    ...
    params['order'] = 'desc';
          ...
          if(this.searchForm.dengjiriqiend!='' && this.searchForm.dengjiriqiend!=undefined){
           ...
           if(this.searchForm.jiankangzhibiao!='' && this.searchForm.jiankangzhibiao!=undefined){
            params['jiankangzhibiao'] = this.searchForm.jiankangzhibiao
          }
      this.$http({
        url: "shentijiankang/page",
        method: "get",
        params: params
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	shentijiankangstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'shentijiankang/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
